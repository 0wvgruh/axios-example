...
function UserList({ onUserClick }) { 
  const [users, setUsers] = useState([]); // useState: users 상태를 정의하여 사용자 목록 데이터를 저장합니다.

  useEffect(() => { //  useEffect: 컴포넌트가 처음 렌더링될 때 한 번 실행되어 axios로 API(https://jsonplaceholder.typicode.com/users)에서 사용자 목록을 가져옵니다. 
        ...
        {users.map(user => (
          <li
            ...
            onClick={() => onUserClick(user.id)}
             // onUserClick: onUserClick은 부모 컴포넌트에서 전달된 함수로, 사용자가 목록의 항목을 클릭할 때 호출
