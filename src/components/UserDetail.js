...
function UserDetail({ userId }) {
  ...
  useEffect(() => {// useEffect: userId가 바뀔 때마다 실행되어, axios로 해당 사용자의 상세 정보를 요청합니다.
    if (!userId) return;
    axios.get(`https://jsonplaceholder.typicode.com/users/${userId}`)// userId Prop: userId는 부모 컴포넌트에서 선택된 사용자 ID를 전달받는 prop입니다.
  ...
  return ( // userId가 있을 경우, axios를 통해 사용자의 상세 데이터를 가져와 화면에 출력합니다.
    <div className={styles.container}>
      ...
      {user ? ( 
        ...
        <p>Loading...</p> // // 로딩 메시지: user 값이 null일 경우에는 "Loading..." 메시지가 출력됩니다.
      )}
    </div>
  );
}
